...
namespace Data_Layer
{
    public class Telemedicine_Database : IDatabase
    {
        ...
        public void Get_ECG(string CPRNumber, string Name, string Address, DateTime Date, List<List<double>> ECG, int Pulse, int HRV)
        {
           try
            {
                _connection.Open();
                ...
                using (SqlCommand command = new SqlCommand("Select CPR, Name, Address, Date, ECGMeasure, Pulse, HRV from EKGDATA WHERE CPR = CPR from EKGDATA)", _connection)) //Mangler de rigtige navne, og hvordan der skal vælges det korrekte
                {
